...
from labagent.config import ConfigError, load_config
from labagent.hooks import HookConfigError, HookEngine, load_hooks
from labagent.permissions import PermissionMode


def main() -> None:
    # 先确保 .labagent/ 目录存在，否则下面写 debug.log 会因目录不存在而崩溃
    Path(".labagent").mkdir(parents=True, exist_ok=True)
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s %(name)s %(message)s",
        filename=".labagent/debug.log",
        filemode="w",
    )

    parser = argparse.ArgumentParser(prog="labagent", description="LabAgent AI coding assistant")
    parser.add_argument(
        "--mode",
    ...
    if args.remote:
        from labagent.remote import RemoteServer
    ...
    from labagent.app import LabAgentApp
    from labagent.driver import NoAltScreenDriver
...
async def _run_prompt(config, permission_mode, hook_engine, prompt: str, output_format: str = "text") -> None:
    ...
    from labagent.client import create_client, resolve_context_window
    from labagent.conversation import ConversationManager
    from labagent.memory.instructions import load_instructions
    ...
    from labagent.tools import create_default_registry
