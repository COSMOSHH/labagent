...
from labagent.conversation import ConversationManager, ToolResultBlock, ToolUseBlock
from labagent.conversation import ThinkingBlock as ConvThinkingBlock
from labagent.memory.auto_memory import MemoryManager
from labagent.permissions import (
    Decision,
    PermissionChecker,
    PermissionMode,
)
from labagent.hooks import HookContext, HookEngine, ToolRejectedError
from labagent.hooks.engine import HookNotification
from labagent.prompts import build_environment_context, build_plan_mode_reminder, build_system_prompt
from labagent.tools import ToolRegistry
from labagent.tools.base import (
    MAX_OUTPUT_CHARS,
    StreamEnd,
    StreamEvent,
    TextDelta,
...
class Agent:
    ...
    def _transcript_path(self) -> str:
        if self.session_id:
            return str(Path(self.work_dir) / ".labagent" / "sessions" / f"{self.session_id}.jsonl")
        return ""
    ...
    def _get_plan_path(self) -> Path:
        ...
        plans_dir = Path(self.work_dir) / ".labagent" / "plans"
        ...
        if self.permission_checker:
            ...
            if decision.effect == "ask":
                ...
                if response == PermissionResponse.ALLOW_ALWAYS:
                    from labagent.permissions.rules import Rule, extract_content
    ...
    def _maybe_persist_or_truncate(self, tool_use_id: str, text: str) -> str:
        from labagent.context.manager import (
