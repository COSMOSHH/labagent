...
from labagent.commands import (
    CommandContext,
    CommandRegistry,
    complete,
    parse_command,
)
from labagent.commands.completion import CompletionPopup
from labagent.commands.handlers import register_all_commands
from labagent.config import MCPServerConfig, ProviderConfig
from labagent.hooks import HookContext, HookEngine, load_hooks
from labagent.conversation import ConversationManager, Message
from labagent.mcp import ConnectResult, MCPManager
from labagent.memory import (
    MemoryManager,
    Session,
    SessionManager,
...
from labagent.permissions import (
...
from labagent.agents.task_manager import TaskManager
from labagent.agents.trace import TraceManager
from labagent.agents.notification import inject_task_notifications
from labagent.commands.handlers.tasks import create_tasks_command
from labagent.skills.executor import SkillExecutor
from labagent.skills.loader import SkillLoader
from labagent.commands.handlers.skill_register import register_skill_commands
from rich.text import Text as RichText
from textual.theme import Theme
from labagent.cache import FileCache
from labagent.tools import ToolRegistry, create_default_registry
from labagent.tools.agent_tool import AgentTool
from labagent.tools.ask_user import AskUserEvent, AskUserTool
from labagent.tools.impl.tool_search import ToolSearchTool
from labagent.tools.install_skill import InstallSkillTool
from labagent.tools.load_skill import LoadSkill
from labagent.worktree.cleanup import start_stale_cleanup_task
from labagent.worktree.manager import WorktreeManager
from labagent.commands.handlers.worktree import create_worktree_command
from labagent.teammate_tree import TeammateTree
...
_SKIP_DIRS = {".git", "node_modules", ".venv", "__pycache__", ".labagent", "build", ".gradle"}
...
class ChatInput(TextArea):
    ...
    def load_history(self, work_dir: str) -> None:
        self._history_file = Path(work_dir) / ".labagent" / "history"
...
class LabAgentApp(App):
    ...
    def __init__(
        ...
        from labagent.config import SandboxAppConfig
    ...
    def on_mount(self) -> None:
        self.register_theme(_LABAGENT_THEME)
        self.theme = "labagent"
        if len(self.providers) == 1:
    ...
    def _select_provider(self, provider: ProviderConfig) -> None:
        ...
        checker = PermissionChecker(
            ...
            rule_engine=RuleEngine(
                user_rules_path=home / ".labagent" / "permissions.yaml",
                project_rules_path=Path(work_dir) / ".labagent" / "permissions.yaml",
                local_rules_path=Path(work_dir) / ".labagent" / "permissions.local.yaml",
            ),
        ...
        if self._sandbox_cfg.enabled:
            from labagent.sandbox import SandboxConfig, create_sandbox
            os_sandbox = create_sandbox()
            if os_sandbox and os_sandbox.available():
                sandbox_config = SandboxConfig(
                    allow_write=[work_dir, "/tmp"],
                    deny_write=[
                        f"{work_dir}/.labagent/config.yaml",
                        f"{work_dir}/.labagent/permissions.local.yaml",
                    ],
        ...
        from labagent.filehistory import FileHistory
        ...
        from labagent.tools.exit_plan_mode import ExitPlanModeTool
        ...
        # --- Worktree 系统初始化 ---
        from labagent.config import WorktreeConfig
        ...
        from labagent.tools.enter_worktree import EnterWorktreeTool
        from labagent.tools.exit_worktree import ExitWorktreeTool
        ...
        # --- Agent 团队系统初始化 ---
        from labagent.teams.manager import TeamManager
        from labagent.tools.team_create import TeamCreateTool
        from labagent.tools.team_delete import TeamDeleteTool
        ...
        from labagent.commands.handlers.trace import create_trace_command
        ...
        from labagent.tools.synthetic_output import SyntheticOutputTool
    ...
    async def _prefetch_relevant_memories(self, query: str) -> str:
        ...
        async def selector(system_prompt: str, user_message: str) -> str:
            from labagent.tools.base import StreamEnd, TextDelta
    ...
    async def _show_plan_approval(self) -> None:
        from labagent.plan_dialog import InlinePlanWidget
    ...
    def on_inline_plan_widget_responded(
        ...
        from labagent.plan_dialog import InlinePlanWidget, PlanChoice
        from labagent.prompts import build_plan_mode_exit_reminder
    ...
    async def _handle_askuser(self, event: AskUserEvent) -> None:
        from labagent.askuser_dialog import InlineAskUserWidget
    ...
    async def _handle_permission_request(self, request: PermissionRequest) -> None:
        from labagent.permission_dialog import InlinePermissionWidget
