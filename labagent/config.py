...
class ProviderConfig:
    ...
    def resolve_api_key(self) -> str:
        if self.api_key:
            ...
            return "" if _ENV_VAR_RE.search(resolved) else resolved
...
def load_dotenv(path: Path) -> None:
    """Load project values without overriding existing environment variables."""
    if not path.is_file():
        return
...
def load_config(path: Path | None = None) -> AppConfig:
    ...
    home = Path.home()
    candidates = [
        home / ".labagent" / "config.yaml",
        cwd / ".labagent" / "config.yaml",
        cwd / ".labagent" / "config.local.yaml",
    ]
    ...
    if merged is None:
        raise ConfigError(
            "No config file found. Expected .labagent/config.yaml "
            "in project or ~/.labagent/config.yaml"
        )
    return merged
