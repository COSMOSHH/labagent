...
def _session_path(labagent_dir: Path) -> Path:
    return labagent_dir / SESSION_FILENAME
...
def save_worktree_session(
    ...
    path = _session_path(labagent_dir)
    if session is None:
...
def load_worktree_session(labagent_dir: Path) -> WorktreeSession | None:
    path = _session_path(labagent_dir)
    if not path.exists():
        return None
    try:
