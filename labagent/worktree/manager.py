...
from labagent.worktree.changes import (
    CleanupResult,
    Changes,
    count_worktree_changes,
    has_worktree_changes,
)
from labagent.worktree.models import Worktree, WorktreeSession
from labagent.worktree.session import load_worktree_session, save_worktree_session
from labagent.worktree.setup import perform_post_creation_setup
from labagent.worktree.slug import flatten_slug, validate_slug
...
class WorktreeManager:
    def __init__(
        ...
        self.worktree_dir = worktree_dir or str(
            Path(repo_root) / ".labagent" / "worktrees"
        )
        self._labagent_dir = Path(repo_root) / ".labagent"
    ...
    async def enter(self, name: str) -> WorktreeSession:
        ...
        save_worktree_session(self._labagent_dir, session)
        return session
    ...
    async def exit(
        ...
        save_worktree_session(self._labagent_dir, None)
    ...
    def restore_session(self) -> WorktreeSession | None:
        session = load_worktree_session(self._labagent_dir)
        if session is None:
            return None
